...
export default async function handler(req, res) {
  ...
  const excluded = String(req.query.exclude || '').split(',').map(normalizeAnswer).filter(Boolean).slice(0, 80);
  ...
  const schema = {
    ...
    properties: { questions: { type: 'array', minItems: config.count, maxItems: config.count, items: {
      type: 'object', additionalProperties: false,
  ...
  try {
    const response = await fetch(OPENAI_URL, {
      ...
      body: JSON.stringify({
        ...
        input: prompt,
        text: { format: { type: 'json_schema', name: 'crossword_questions', strict: true, schema } },
      }),
    });

    const payload = await response.json();
    if (!response.ok) return send(res, 502, { error: 'question_generation_failed' });
    ...
    return send(res, 200, { source: 'openai-web-verified', level, questions });
  } catch {
