...
function render(){
  ...
  function activateWord(word,focusKey=null){
    ...
    inputForKey(focusKey||wordCells.get(word)[0])?.focus();
  }
  ...
  function moveWithinActive(input,delta,skipFilled=false){
    ...
    while(next>=0&&next<keys.length){
      ...
      if(!target)return;
      if(!skipFilled||!target.value){target.focus();return;}
      next+=delta;
    }
  }

  game.grid.forEach((row,r)=>row.forEach((letter,c)=>{
    if(!letter){const d=document.createElement('div');d.className='celda bloqueada';board.appendChild(d);return;}
    const wrap=document.createElement('div');
    wrap.className='casilla';
    const key=cellKey(r,c);
    if(starts.has(key)){const n=document.createElement('span');n.className='numero-casilla';n.textContent=starts.get(key);wrap.appendChild(n);}
    ...
    input.addEventListener('click',()=>{const word=chooseWordAt(r,c,true);if(word)activateWord(word,key);});
    input.addEventListener('focus',()=>{if(activeWord&&wordCells.get(activeWord)?.includes(key))return;const word=chooseWordAt(r,c,false);if(word)activateWord(word,key);});
    input.addEventListener('input',e=>{e.target.value=e.target.value.toUpperCase().replace(/[^A-ZÑ]/g,'');updateProgress();if(e.target.value)moveWithinActive(e.target,1,true);});
    input.addEventListener('keydown',e=>{if(e.key==='Backspace'&&!e.target.value){e.preventDefault();moveWithinActive(e.target,-1,false);const focused=document.activeElement;if(focused?.classList?.contains('celda')){focused.value='';updateProgress();}}});
  ...
  const section=(title,words)=>`<section class="grupo-pistas"><h3>${title}</h3>${words.map(w=>`<button class="pista" type="button" data-row="${w.row}" data-col="${w.col}" data-dir="${w.dir}"><strong>${w.number}.</strong> ${w.q} <small>(${w.a.length})</small></button>`).join('')}</section>`;
  clues.innerHTML=section('Horizontales',horizontal)+section('Verticales',vertical);
  clues.querySelectorAll('.pista').forEach(btn=>btn.addEventListener('click',()=>{const word=game.words.find(w=>w.row===Number(btn.dataset.row)&&w.col===Number(btn.dataset.col)&&w.dir===btn.dataset.dir);if(word)activateWord(word);}));
...
function verify(){
  let wrong=0,empty=0;
  document.querySelectorAll('input.celda').forEach(x=>{x.classList.remove('ok','error');if(!x.value){empty++;return;}if(x.value===x.dataset.answer)x.classList.add('ok');else{x.classList.add('error');wrong++;}});
  if(!wrong&&!empty){score+=level==='facil'?200:level==='medio'?400:600;localStorage.setItem('nexar_crossword_score',score);$('#puntaje').textContent=score;$('#mensaje').textContent='🎉 ¡Crucigrama completo!';}
  else $('#mensaje').textContent=wrong?`Hay ${wrong} letra(s) incorrecta(s).`:`Faltan ${empty} casilla(s).`;
}

$('#niveles').addEventListener('click',e=>{const b=e.target.closest('button[data-nivel]');if(!b)return;level=b.dataset.nivel;document.querySelectorAll('#niveles button').forEach(x=>x.classList.toggle('activo',x===b));start();});
